...
Chat 3 picked up exactly where Chat 2 left off: the top-up job's live
test was mid-verification, `runTopup` was not yet exported, and a
`test-topup-dedup.ts` script sat uncommitted. This section will be
written in full, teaching-style detail (matching the standard set by
the Chat 1/2 reconstruction above) once Chat 3 concludes — not
incrementally turn-by-turn, to avoid burning tokens mid-session on
documentation instead of the actual build. Placeholder only until then.
